...
1. **本地数据层的稳定**：`course.json` / `personal-schedule.json` 是静态快照，导出
   与刷新失败时不会清空上次成功快照，页面同时展示失败时间与原因；执行器失败也只
   写事件、不破坏课表。这一层已经可用、可测试（testlab 场景覆盖）。
2. **真实教务接口的稳定**：HDU 正方教务是学校侧维护的 Web 系统，以下不稳定因素不
   在本仓库可控制范围内：
   - 登录会话过期/被踢下线、接口随机限流或 403（需要有效会话的来源）；
   - 不同时间段（非选课期）接口关闭或返回“当前不属于选课阶段”；
   - 接口字段/权限变更导致抓包逻辑失效。

   对策（已实现或按 P4 排期）：登录失败按新教务/CAS 顺序重试并保存 cookies；接口被
   拒时给出可诊断的错误与离线快照；后续补“重登退避 + 轮询频率限制 + 接口变更诊断”。

结论：**不能承诺真实教务接口“永不失败”**——任何调用方都做不到。本仓库保证的是
“失败可见、可诊断、不影响本地数据和已授权执行的安全边界”；真实抢课/退课能否成功
取决于登录会话与选课期开放状态，需以真实环境实测为准。

## 七、P4 稳定强化落地记录（2026-08-21）

1. **执行器登录过期自动重登 + 单次重试（已实现）**：`executor` 在选/退/蹲课过程中
   遇到“可能登录过期 / 统一身份认证”时自动重新登录一次并重试原动作
   （`executor.go` 的 `execCourseAction`/`relogin`，重登会保留学生选课配置
   `ClientBodyConfig` 并在新会话下刷新学生信息）。mock 测试覆盖：
   - RunOnce 首次过期 → 重登 → 选课成功；
   - RunOnce 持续过期 → 标记失败（不会把过期当成功）；
   - StartWait 查询余量过期 → 重登 → 下一轮选课成功。
2. **蹲课轮询频率限制与瞬时失败韧性（已实现）**：`StartWait` 在查询余量遇瞬时
   错误时不再立刻放弃该门课，而是保留课程继续蹲，并对连续失败做指数退避
   （间隔翻倍、上限 600s，封顶后不再无限增长；`waitInterval`）；蹲课中选课失败
   也保留课程继续等待。仅在重登后仍未收敛时通过事件告知用户。测试覆盖：瞬时
   失败后仍选课成功、退避数值表（含封顶与用户超限覆盖）。
3. **执行过程实时状态上报（已实现）**：`executor` 通过 `SetOnEvent` 在每门课
   running/final 时逐步发事件；Smart Agent 在执行器启动时接入该回调，把事件写入
   内存事件流，`/api/execution/status` 在执行中即返回每门课的进度（running 计入
