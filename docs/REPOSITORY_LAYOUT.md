...
3. Run `powershell -ExecutionPolicy Bypass -File scripts/release-integrity-check.ps1` to verify the ZIP sidecar checksum and every manifest file's SHA-256 inside the archive.
4. Run the release main smoke test and the Smart Agent checks from the generated package. If the default port is busy, pass `-Port <free-port>` to `release-main-smoke.ps1` and set `HDU_SMART_AGENT_PORT` for the Smart Agent checks.
5. Upload the generated ZIP and its `.zip.sha256` sidecar as GitHub Release assets for the matching tag.

The tag, `VERSION`, package directory, ZIP name, manifest version, and sidecar checksum must match. A release asset is separate from the Git file tree, so a successful `git push` does not upload files under `release/`.
